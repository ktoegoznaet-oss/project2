...
router.post('/upload', auth, adminOnly, upload.single('audio'), async (req, res, next) => {
  try {
    ...
    const { title, artist, genre, order_price } = req.body;
    if (!title || !artist) {
      return res.status(400).json({ error: 'Укажите название и исполнителя' });
    }
    ...
    const result = await query(
      ...
      [title, artist, genre || 'other', filePath, hash, parseFloat(order_price) || 100, parseInt(req.body.duration) || 0]
    );
